...
train_transformed["above_77k"] = (train_transformed["critical_temp"] > 77).astype(int)

# Check the number of records in each class
print("Number of records in each 77 K class:")
print(train_transformed["above_77k"].value_counts().sort_index())

# Check the proportion of records in each class
print("\nProportion of records in each 77 K class:")
print(train_transformed["above_77k"].value_counts(normalize=True).sort_index())

# Save the transformed dataset
train_transformed.to_csv("data/processed/train_transformed.csv", index=False)
